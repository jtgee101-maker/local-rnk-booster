...
const nurtureSequence = [
  {
    ...
    template: (businessName) => `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <a href="https://localrank.ai/Pricing" style="display: inline-block; background: #c8ff00; color: #000; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: bold; margin: 20px 0;">Fix My GMB Profile Now</a>
  ...
  },
  {
    ...
    template: (businessName) => `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <p><strong>What's the difference?</strong> They optimized their GMB profiles. That's it.</p>
        <a href="https://localrank.ai/Pricing" style="display: inline-block; background: #c8ff00; color: #000; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: bold; margin: 20px 0;">Get My Fair Share of Calls</a>
      </div>
    `
  },
  {
    ...
    template: (businessName) => `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <p>This is it. After tonight, our 82% discount goes away.</p>
        ...
        <a href="https://localrank.ai/Pricing" style="display: inline-block; background: #c8ff00; color: #000; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: bold; margin: 20px 0;">Claim 82% Off Now</a>
  ...
  },
  {
    ...
    template: (businessName) => `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <a href="https://localrank.ai/Pricing" style="display: inline-block; background: #c8ff00; color: #000; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: bold; margin: 20px 0;">Claim My Spot Now</a>
      </div>
    `
  },
  {
    ...
    template: (businessName, healthScore) => `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <a href="https://localrank.ai/Pricing" style="display: inline-block; background: #c8ff00; color: #000; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: bold; margin: 20px 0;">Get Started</a>
...
Deno.serve(async (req) => {
  try {
    ...
    const activeNurtures = await base44.asServiceRole.entities.LeadNurture.filter({ status: 'active' });
    const dueNurtures = activeNurtures.filter(n => n.next_email_date && new Date(n.next_email_date) <= new Date());
    ...
    for (const nurture of dueNurtures) {
      try {
        ...
        if (nextStep > nurture.total_steps) {
          await base44.asServiceRole.entities.LeadNurture.update(nurture.id, { status: 'completed' });
          continue;
        }
        ...
        if (nextSequence) nextEmailDate.setHours(nextEmailDate.getHours() + nextSequence.delay_hours);
    ...
    return Response.json({ success: true, processed, errors, total: dueNurtures.length });
  } catch (error) {
