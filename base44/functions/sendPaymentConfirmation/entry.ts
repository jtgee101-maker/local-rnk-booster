...
Deno.serve(async (req) => {
  try {
    ...
    const { email, businessName, amount, productName, invoiceId } = await req.json();
    ...
    const emailBody = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0f; color: #fff; padding: 40px 20px;">
        <div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); border: 1px solid #333; border-radius: 16px; padding: 30px;">
          ...
          <h2 style="color: #c8ff00; text-align: center; margin-top: 0; font-size: 26px;">Payment Confirmed</h2>
          <p style="color: #ccc; line-height: 1.6; text-align: center; font-size: 16px;">
            Hi ${businessName || 'there'}! Your payment for <strong>${productName || 'LocalRank Service'}</strong> has been received.
          </p>
          <div style="background: rgba(200, 255, 0, 0.1); border: 2px solid rgba(200, 255, 0, 0.3); border-radius: 12px; padding: 20px; margin: 25px 0;">
            <p style="margin: 0 0 5px 0; color: #999; font-size: 12px; text-align: center;">AMOUNT PAID</p>
            <div style="font-size: 36px; color: #c8ff00; font-weight: bold; text-align: center;">$${amount || 0}</div>
            ${invoiceId ? `<p style="margin: 10px 0 0 0; color: #666; font-size: 12px; text-align: center;">Invoice: ${invoiceId}</p>` : ''}
          </div>
    ...
    await base44.asServiceRole.entities.EmailLog.create({
      ...
      status: 'sent',
      metadata: { business_name: businessName, amount, product: productName, invoice_id: invoiceId }
    });
