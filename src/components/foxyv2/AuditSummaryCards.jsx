...
export default function AuditSummaryCards({ auditData, onExpand }) {
  const cards = [
    ...
    },
    {
      ...
      value: auditData.heatmap?.data?.visibilityScore || 0,
      suffix: '%',
      status: (auditData.heatmap?.data?.visibilityScore || 0) >= 70 ? 'healthy' : 'warning',
      insight: `${auditData.heatmap?.data?.weakZones || 0} weak zones | ${auditData.heatmap?.data?.gridSize || 0} locations checked`,
    ...
    },
    {
      ...
      icon: Users,
      title: 'Average Rank',
      value: auditData.heatmap?.data?.averageRank ? `#${Math.ceil(auditData.heatmap.data.averageRank)}` : '#7',
      suffix: '',
      status: (auditData.heatmap?.data?.averageRank || 7) <= 3 ? 'healthy' : 'warning',
      insight: `${auditData.heatmap?.data?.strongZones || 0} strong zones in service area`,
