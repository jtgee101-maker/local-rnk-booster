...
export default function AuditSummaryCards({ auditData, onExpand }) {
  const cards = [
    ...
    },
    {
      ...
      insight: `${auditData.heatmap?.weakZones || 0} weak zones | ${auditData.heatmap?.gridSize || 0} locations checked`,
    ...
    },
    {
      ...
      icon: Users,
      title: 'Average Rank',
      value: auditData.heatmap?.averageRank ? `#${Math.ceil(auditData.heatmap.averageRank)}` : '#7',
      suffix: '',
      status: (auditData.heatmap?.averageRank || 7) <= 3 ? 'healthy' : 'warning',
      insight: `${auditData.heatmap?.strongZones || 0} strong zones in service area`,
